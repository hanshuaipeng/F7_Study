    ...
    { 
        if(!bHz)
        {
	        ...
	        {      
		        ...
		        else LTDC_ShowChar(x,y,*str,size,mode);//有效部分写入 
				str++; 
		        x+=size/2; //字符,为全字的一半 
	        }
        }else//中文 
...
void Show_Str_Mid(uint16_t x,uint16_t y,uint8_t*str,uint8_t size,uint8_t len)
{
	...
	strlenth*=size/2;
	if(strlenth>len)Show_Str(x,y,lcdltdc.width,lcdltdc.height,str,size,1);
	else
	{
		strlenth=(len-strlenth)/2;
	    Show_Str(strlenth+x,y,lcdltdc.width,lcdltdc.height,str,size,1);
	}
}   
